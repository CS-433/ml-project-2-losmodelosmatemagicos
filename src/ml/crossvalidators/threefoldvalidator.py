...
class ThreeCrossValidator(CrossValidator):
    ...
    def xval(self, sequences: list, labels: list, demographics: dict) -> dict:
        ...
        for f in range(self._settings["ml"]["nfolds"]):
            train_index, test_index = self._splitter.split(
                sequences, demographics["stratifier_col"]
            )
            print(" test index: {}".format(test_index[0:5]))
            logging.debug(
                "    length train: {}, length test: {}".format(
            ...
            oversampler_train = [
                demographics["oversampler_col"][tidx] for tidx in train_index
            ]
            demographics_train = {}
            ...
            # Inner loop
            x_resampled, y_resampled, idx_resampled = self._sampler.sample(
                x_train, oversampler_train, y_train, demographics_train
            )
            ...
            if model.get_settings()["save_best_model"]:
                train_x, val_x, train_y, val_y = train_test_split(
                    ...
                    y_resampled,
                    test_size=0.1,
                    random_state=self._settings["seeds"]["splitter"],
                )
            ...
            print(
                "Y train balance: {}, Y TEST balance: {}".format(
                    Counter(train_y), Counter(y_test)
                )
            )
            ...
            for id_d in demographics.keys():
                if "_col" not in id_d:
                    ...
                    fairness_results = self._scorer.get_fairness_scores(
                        y_test, y_pred, y_proba, test_demo, self._fairness_metrics
                    )
    ...
    def save_results(self, results):
        ...
        path = path / (
            self._notation
            + "_m"
            + self._model_notation
            + "_l"
            + str(self._settings["data"]["adjuster"]["limit"])
            + ".pkl"
        )
