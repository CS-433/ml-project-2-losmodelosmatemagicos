...
def mask_input_and_labels(
    ...
    # Real question! We want to mask SEP or not?
    # Do not mask special tokens
    inp_mask[
        encoded_texts <= len(special_token_dict) - 1
    ] = False  # The special_token_dict starts at 0
    ...
    inp_mask_2mask = inp_mask & (
        np.random.rand(*encoded_texts.shape) < ratio_seq_masked
    )
    encoded_texts_masked[inp_mask_2mask] = special_token_dict["[MASK]"]
    ...
    inp_mask_2random = inp_mask_2mask & (
        np.random.rand(*encoded_texts.shape) < (ratio_random_seq / ratio_seq_masked)
    )
    encoded_texts_masked[inp_mask_2random] = np.random.randint(
        len(
            special_token_dict
        ),  # low = len(special_token_dict) ( included, is not a problem cuz special_token_dict starts at 0 )
        np.max(encoded_texts) + 1,  # high = last_token + 1 (not included)
