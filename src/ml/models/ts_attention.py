...
class TimestepAttentionModel(Model):
    ...
    def _load_settings(self):
        ...
        self._model_settings["epochs"] = self._settings["ml"]["epochs"]

    def _format(self, x: list, y: list) -> Tuple[list, list]:
        # y needs to be one hot encoded
        x_vector = pad_sequences(
            x,
            padding="post",
            value=self._model_settings["padding_value"],
            maxlen=self._maxlen,
            dtype=float,
        )
    ...
    def _get_csvlogger_path(self) -> str:
        csv_path = "../experiments/{}{}/{}/logger/{}/".format(
        ...
        csv_path += "ct{}_nlayers{}_ncells{}_flatten{}".format(
            self._model_settings["cell_type"],
            self._model_settings["n_layers"],
            self._model_settings["n_cells"],
            self._model_settings["flatten"],
        )
        csv_path += "_drop{}_optim{}_loss{}_bs{}_ep{}".format(
            self._model_settings["dropout"],
            self._model_settings["optimiser"],
            self._model_settings["loss"],
    ...
    def _get_model_checkpoint_path(self) -> str:
        path = "../experiments/{}{}/{}/logger/{}/".format(
            self._experiment_root,
            self._experiment_name,
            self._outer_fold,
            self._notation,
    ...
    def load_model_weights(self, x: np.array, checkpoint_path: str):
        ...
        if os.path.exists(PurePath(temporary_path)):
            ...
            copytree(
                PurePath(checkpoint_path), PurePath(temporary_path), dirs_exist_ok=True
            )
    ...
    def _init_model(self, x: np.array):
        ...
        full_features = layers.Masking(
            mask_value=self._model_settings["padding_value"], name="masking_prior"
        )(input_layer)

        selfattention_features = self.self_attention(full_features)
        print(
            "       sa {}".format(selfattention_features.shape)
        )  # expect shape of None, 900, 10
        print("       ff {}".format(full_features.shape))  # expect shape of 900

        concatenated = layers.Concatenate(axis=2)(
            [full_features, selfattention_features]
        )
        print(
            "       concatenated {}".format(concatenated.shape)
        )  # expect shape of 900 x (lstm cells + 1)
        ...
        concatenated = self._get_rnn_layer(
            return_sequences=False, l=self._model_settings["n_layers"] - 1
        )(concatenated)

        classification_layer = layers.Dense(self._n_classes, activation="softmax")(
            concatenated
        )
