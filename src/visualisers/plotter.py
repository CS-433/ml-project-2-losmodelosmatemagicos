...
class Plotter:
    ...
    def __init__(self, settings: dict):
        ...
        self._root_plot_path = "../experiments/{}/figures/".format(
    ...
    def _update_csv(self, csv_data: dict):
        ...
        """
        csv_path = "../experiments/{}/{}".format(
            self._settings["csv"]["folder"], self._settings["csv"]["file"]
        )
        try:
        ...
        for experiment in csv_data:
            for metric in csv_data[experiment]:
                ...
                mask = (current_results["key"] == current_key) & (
                    current_results["metric"] == metric
                )
                temp = current_results[mask]
                if len(temp) == 1:
                    for column in csv_data[experiment][metric]:
                        current_results.loc[mask, column] = csv_data[experiment][
                            metric
                        ][column]
                else:
    ...
    def _savefig(self, name: str, subfolder=""):
        ...
        """
        path = "{}{}{}.svg".format(self._root_plot_path, subfolder, name)
