...
class NonNestedPlotter(Plotter):
    ...
    def _get_onemetric_onedemographics_overall_barplots(
        ...
        for demographic_type in self._settings["data"]["demographics"]:
            for metric in self._settings["data"]["fairness_metrics"]:
                ...
                for i_e, experiment in enumerate(results):
                    ...
                    demo_scores = {
                        r_key: []
                        for r_key in results[experiment]["result"][i_fold][
                            demographic_type
                        ][metric]
                    }
                    for d in demo_scores:
                        ...
                        while i_fold in results[experiment]["result"]:
                            if (
                                d
                                not in results[experiment]["result"][i_fold][
                                    demographic_type
                                ][metric]
                            ):
                                i_fold += 1
                                continue
                            if (
                                results[experiment]["result"][i_fold][demographic_type][
                                    metric
                                ][d]
                                == -1
                            ):
    ...
    def _get_fairness_barplots(self, resultconfigs: dict):
        if self._settings["overall"]:
            ...
            self._get_onemetric_onedemographics_overall_barplots(
