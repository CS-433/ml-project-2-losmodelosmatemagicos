...
class CSVViewer(Plotter):
    ...
    def _read_csv(self):
        csv_path = "{}/{}".format(
            self._csv_settings["folder"], self._csv_settings["file"]
        )
        self._csv = pd.read_csv(csv_path, sep="\t", index_col=0)

        if len(self._csv_settings["filtering_columns"]) > 0:
            include_re = re.compile(
                "({})".format("|".join(self._csv_settings["filtering_columns"]))
            )
            temp = [c for c in self._csv.columns]
            mask = [
                True if len(include_re.findall(temp[idx])) > 0 else False
                for idx in range(len(temp))
            ]
            columns = self._csv_settings["fixed"] + [
                self._csv.columns[idx] for idx in range(len(temp)) if mask[idx]
            ]
        ...
        if len(self._csv_settings["filtering_experiments"]) > 0:
            for col in self._csv_settings["filtering_experiments"]:
                self._csv = self._csv[
                    self._csv[col].isin(
                        self._csv_settings["filtering_experiments"][col]
                    )
                ]
    ...
    def _sort(self):
        if "baseline" in self._settings["csv"]["sorting"]:
            self._csv["baseline"] = self._csv["key"].apply(
                lambda x: self._name_baseline(x)
            )
        else:
    ...
    def _colour_cell_baseline(
        self, columns: list, baseline: dict, row: dict, ascending: bool
    ) -> str:
        colours = []
                ...
                if row["baseline"]:
                    ...
                    if ascending and "equalodds" not in col:
                        if baseline[col] < row[col]:
                            colours.append(
                                "background-color: darkkhaki; text_wrap: True"
                            )
                        else:
                            colours.append("background-color: white; text_wrap: True")
                    else:
