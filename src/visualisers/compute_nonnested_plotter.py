...
class NonNestedPlotter(Plotter):
    ...
    def _crawl_files(self):
        ...
        """
        results_files = []
        experiment_path = "../experiments/{}".format(
    ...
    def _load_data(self, results_files: list, configs_files: list):
        ...
        for result in results_files:
            ...
            try:
                ...
                results[key] = {"result": result_data, "config": config_data}
            except IndexError:
    ...
    def _get_fold_scorer(self, n_classes: int, metrics: list):
        ...
        """
        new_settings = dict(self._settings)
        new_settings["ml"] = {"scorer": {"scoring_metrics": metrics}}
        if n_classes == 2:
            return BinaryClfScorer(new_settings)
        else:
            print("not implemented for multi class yet")
    ...
    def _get_fairness_data(self, resultsconfig: dict):
        ...
        for experiment in resultsconfig:
            ...
            while fold in results:
                y_true = y_true + [
                    results["y"][idx] for idx in results[fold]["test_index"]
                ]
                y_pred = y_pred + results[fold]["y_pred"]
                y_prob = y_prob + [yy for yy in results[fold]["y_proba"]]
                for demo in demo_list:
                    demo_list[demo] = demo_list[demo] + [
                        results["demographics"][demo][idx]
                        for idx in results[fold]["test_index"]
                    ]
                fold += 1
    ...
    def _get_onemetric_onedemographics_overall_boxplots(
        self, ytrues: list, ypreds: list, yprobs: list, demographics: list
    ):
        ...
        for demographic_type in self._settings["data"]["demographics"]:
            for metric in self._settings["data"]["fairness_metrics"]:
                ...
                demo_attributes = np.unique(
                    demographics[list(ytrues.keys())[0]][demographic_type]
                )
                for i_e, experiment in enumerate(ytrues):
                    ...
                    scores = scorer.get_fairness_scores(
                        ytrue, ypred, yprob, demo[demographic_type], [metric]
                    )

                    for d in demo_attributes:
                        if scores[metric][d] == -1:
                            print(
                                "undefined {} for demographic {} for all folds {}".format(
                                    metric, d
                                )
                            )
                            continue
                        ...
                        x += (
                            self._settings["style"]["bar_width"] * len(ytrues)
                        ) + self._settings["style"]["groupspacing"]

                    self._plot_single_barplot(xs, ys, colours[i_e], experiment)
                    e += (
                        self._settings["style"]["bar_width"]
                        + self._settings["style"]["xspacing"]
                    )
                ...
                if self._settings["save"]:
                    self._savefig(
                        "{}_{}_overall_barplot".format(metric, demographic_type),
                        "fairness_metrics",
                    )
    ...
    def _get_onemetric_onedemographics_overall_csv(
        self,
        ytrues: list,
        ypreds: list,
        yprobs: list,
        ...
        for demographic_type in self._settings["data"]["demographics"]:
            for metric in self._settings["data"]["fairness_metrics"]:
                for i_e, experiment in enumerate(ytrues):
                    demo_attributes = np.unique(
                        demographics[experiment][demographic_type]
                    )
                    ...
                    for d in demo_attributes:
                        if scores[metric][d] == -1:
                            ...
                            continue
                        experiment_table[experiment][metric]["overall"] = scores[
                            metric
                        ][d]
        ...
        for demographic_type in self._settings["data"]["demographics"]:
            for metric in self._settings["data"]["fairness_metrics"]:
                ...
                for i_e, experiment in enumerate(resultsconfig):
                    ...
                    for d in demo_scores:
                        ...
                        while i_fold in results:
                            test_demos = [
                                results["demographics"][demographic_type][idx]
                                for idx in results[i_fold]["test_index"]
                            ]
                            ...
                            demo_indices = [
                                idx
                                for idx in range(len(test_demos))
                                if test_demos[idx] == d
                            ]
                            d_indices = [
                                results[i_fold]["test_index"][didx]
                                for didx in demo_indices
                            ]
                            d_demos = [test_demos[didx] for didx in demo_indices]
                            d_trues = [results["y"][didx] for didx in d_indices]
                            d_pbs = [
                                results[i_fold]["y_proba"][didx]
                                for didx in demo_indices
                            ]
                            d_pds = [
                                results[i_fold]["y_pred"][didx] for didx in demo_indices
                            ]
                            # debug
                            debug = [td for td in test_demos if td == d]
                            assert len(debug) == len(
                                d_demos
                            )  # debug to say that indexing worked correctly

                            d_score = scorer.get_fairness_scores(
                                d_trues, d_pds, d_pbs, d_demos, [metric]
                            )
                        ...
                        experiments_details[experiment]["{}_mean".format(d)] = np.mean(
                            fold_scores
                        )
                        experiments_details[experiment]["{}_std".format(d)] = np.std(
                            fold_scores
                        )
                        self._plot_single_boxplot(fold_scores, colours[i_e], x)
                        # plt.scatter([x for _ in fold_scores], fold_scores, color='black')
                        x += (
                            self._settings["style"]["bar_width"] * len(results)
                        ) + self._settings["style"]["groupspacing"]
                ...
                if self._settings["print"]:
                    ...
                    print("*" * 40)
                    print(
                        "demographics: {}, metric: {}".format(demographic_type, metric)
                    )
                ...
                if self._settings["save"]:
                    self._savefig(
                        "{}_{}_fold_boxplot".format(metric, demographic_type),
                        "fairness_metrics",
                    )
                if self._settings["show"]:
                    plt.show()
    ...
    def _get_onemetric_onedemographics_fold_csv(
        self, resultsconfig: dict, csv_data: dict
    ):
        experiment_table = dict(csv_data)
        for demographic_type in self._settings["data"]["demographics"]:
            for metric in self._settings["data"]["fairness_metrics"]:
                for i_e, experiment in enumerate(resultsconfig):
                    ...
                    for d in demo_scores:
                        ...
                        experiment_table[experiment][metric][
                            "{}_{}-mean".format(demographic_type, d)
                        ] = np.mean(fold_scores)
                        experiment_table[experiment][metric][
                            "{}_{}-std".format(demographic_type, d)
                        ] = np.std(fold_scores)
    ...
    def _get_combineddemo_barplot(self, resultsconfig: dict):
        ...
        for experiment in resultsconfig:
            ...
            for demographics in self._settings["data"]["combined_demographics"]:
                recombined_demos = [
                    "{}_{}".format(
                        recombined_demos[idx],
                        results["demographics"][demographics][idx],
                    )
            ...
            while (
                fold in results
                and fold < resultsconfig[experiment]["config"]["ml"]["nfolds"]["full"]
            ):
                ...
                demos = demos + [
                    recombined_demos[tidx] for tidx in results[fold]["test_index"]
                ]
                fold += 1
            ...
            for metric in self._settings["data"]["metrics"]:
                ...
                plt.title(
                    "{} for {}".format(
                        metric,
                        " ".join(self._settings["data"]["combined_demographics"]),
                    )
                )
                ...
                plt.bar(
                    x=xs,
                    height=heights,
                    width=self._settings["style"]["bar_width"],
                    color=colours,
                )
                plt.xticks(xs, rds, rotation=self._settings["style"]["rotation"])
                plt.ylim([0, 1])
                plt.title(
                    "{} for combined demographics {} - {}".format(
                        metric,
                        " ".join(self._settings["data"]["combined_demographics"]),
                        experiment,
                    )
                ...
                if self._settings["save"]:
                    self._savefig(
                        "{}_{}_combineddemographics_overall_barplot".format(
                            metric,
                            " ".join(self._settings["data"]["combined_demographics"]),
                        ),
                        "fairness_metrics",
        ...
        for experiment in resultsconfig:
            ...
            for metric in self._settings["data"]["metrics"]:
                for rd in np.unique(recombined_demos):
                    ...
                    experiment_table[experiment][metric][
                        "combinedoverall_{}_mean".format(rd)
                    ] = d_score[metric]
        ...
        for experiment in resultconfigs:
            ...
            for demographic in self._settings["data"]["demographics"]:
                ...
                while fold in results and fold < config["ml"]["nfolds"]["full"]:
                    ys = ys + [
                        results["y"][tidx] for tidx in results[fold]["test_index"]
                    ]
                    ypreds = ypreds + [yp for yp in results[fold]["y_pred"]]
                    yprobs = yprobs + [ypp for ypp in results[fold]["y_proba"]]
                    demos = demos + [
                        results["demographics"][demographic][tidx]
                        for tidx in results[fold]["test_index"]
                    ]
                    fold += 1
                ...
                for metric in self._settings["data"]["metrics"]:
                    ...
                    distance_table = {
                        d: {dd: -1 for dd in unique_demos} for d in unique_demos
                    }
                    score_table = {}
                    for d_attribute in unique_demos:
                        exclude_re = re.compile(
                            "({})".format(
                                "|".join(self._settings["data"]["exclude_demographics"])
                            )
                        )
                        ...
                        d_scores = scorer.get_scores(
                            demos_trues, demos_preds, demos_probs
                        )
                        score_table[d_attribute] = d_scores[metric]

                    for d_i in unique_demos:
                        for d_j in unique_demos:
                            distance_table[d_i][d_j] = np.abs(
                                score_table[d_i] - score_table[d_j]
                            )
                    ...
                    for d_i, d in enumerate(unique_demos):
                        d_scores = [
                            distance_table[d][dd] for dd in distance_table[d] if d != dd
                        ]
                        xs.append(x)
                        xlabels.append(d)
        ...
        if self._settings["overall"]:
            ...
            if self._settings["pairwise"]:
                ...
                if self._settings["show"] or self._settings["save"]:
                    y_trues, y_preds, y_probs, demographics = self._get_fairness_data(
                        resultconfigs
                    )
                    self._get_onemetric_onedemographics_overall_boxplots(
                        y_trues, y_preds, y_probs, demographics
                    )
                if self._settings["print"] or self._settings["dump"]:
                    ...
                    csv_data = self._get_csv_data(resultconfigs)
                    self._get_onemetric_onedemographics_overall_csv(
                        y_trues, y_preds, y_probs, demographics, csv_data
                    )
    ...
    #### Compute equal odds
    def _get_onemetric_onedemographics_fold_equal_odds_print(
        self, resultsconfig: dict, csv_results: dict
    ):
            ...
            for demographic_type in self._settings["data"]["demographics"]:
                ...
                for i_e, experiment in enumerate(resultsconfig):
                    ...
                    if self._settings["print"]:
                        ...
                        print("*" * 40)
                        print(
                            "experiment {} - demographics: {}, metric: equal odds for {}".format(
                                experiment, demographic_type, metric
                            )
                        )
                    for i in range(len(demo_scores)):
                        for j in range(i + 1, len(demo_scores)):
                            ...
                            csv_table[experiment][metric][
                                "{}_equalodds_{}-{}".format(
                                    demographic_type, demo_scores[i], demo_scores[j]
                                )
                            ] = np.abs(
    ...
    def _plot_multiple_metric_boxplots(self, results, metric):
        ...
        for i, key in enumerate(results):
            ...
            if self._settings["print"]:
                ...
                print(
                    "    mean: {}\n    std: {}".format(
                        np.mean(key_plot), np.std(key_plot)
                    )
                )
                print()
        plt.xlim(
    ...
    def test(self, settings):
        ...
        if self._settings["equal_odds"]:
            csv_data = self._get_csv_data(resultconfigs)
            self._get_onemetric_onedemographics_fold_equal_odds_print(
                resultconfigs, csv_data
            )
